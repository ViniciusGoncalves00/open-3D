...
export class Builder {
    public static buildNumberProperty(propertyName: string, property: ObservableField<number>): HTMLElement {
        ...
        template.innerHTML = `
            <div role="row" class="w-full min-h-6 flex items-start justify-center font-normal max-h-64 overflow-auto">
                <div role="label" class="w-2/4 h-full text-sm truncate flex items-center">${propertyName}</div>
                <div role="field" class="w-2/4 h-full"></div>
            </div>
        `;
    ...
    public static buildKeyBindingMouseProperty(
        ...
        label.className = "w-2/4 h-full text-sm truncate flex items-center";
        label.textContent = propertyName;

        const field = document.createElement("button");
        field.className = "w-2/4 h-full text-xs px-1 py-0.5 border border-gray-01 rounded focus:outline-none no-spinner";
